...
exports.pay = functions
  ...
  const payReq = JSON.stringify({
    ...
    redirect_urls: {
      ...
      cancel_url: `https://localist.netlify.com/#/cancel`
    },
    transactions: [{
      amount: {
...
exports.process = functions
  ...
  const r = await paypal.payment.execute(paymentId, payerId, (error, payment) => {
    if (error) {
      console.error(error);
      res.redirect("https://localist.netlify.com/#/error");
    } else {
      if (payment.state === 'approved') {
        console.info('payment completed successfully: ', payment.transactions[0]);
        //console.info('req.custom: : ', payment.transactions[0].custom);
        const date = Date.now();
        const uid = payment.transactions[0].custom;

        if (payment.transactions[0].amount.currency == "GBP") {
            ...
            userRef.update({
                ...
                console.log("Document updated failed: ", err);
            });
        }
        ...
        res.redirect("https://localist.netlify.com");
      } else {
